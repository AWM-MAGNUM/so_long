...
/*   Updated: 2023/03/06 23:00:39 by bel-kase         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_img	img_xpm(void *mlx)
{
	...
	tty.gr_img = mlx_xpm_file_to_image(mlx, "./ground.xpm", &ver, &horz);
	return (tty);
}
...
void	draw_map(t_game *g, int l, int c, char current_char)
{
	...
	{
		mlx_put_image_to_window(g->mlx, g->win, g->img.gr_img, c * 50, l * 50);
		mlx_put_image_to_window(g->mlx, g->win, g->img.player_img, c * 50,
			l * 50);
	}
